...
class WeatherService{
  ...
  getCurrentWeather({required double latitude, required double longitude }) async{
    Response response =await dio.get("$basedUrl/forecast.json?key=$apiKey&q=$latitude,$longitude&days=7&aqi=no&alerts=no");
    try
    {
      WeatherModel weatherModel=WeatherModel.fromJson(response.data);
      print(weatherModel);
